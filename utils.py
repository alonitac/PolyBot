...
def calc_backlog_per_instance(sqs_queue_client, asg_client, asg_group_name):
    while True:
        ...
        asg_size = asg_client.describe_auto_scaling_groups(AutoScalingGroupNames=[asg_group_name])['AutoScalingGroups']
        [0]['DesiredCapacity']
        ...
        cloudwatch.put_metric_data(
            MetricData=[
                {
                    'MetricName': 'shlomigd_backlog_per_instance',
                    'Dimensions': [
                        {
                            'Name': 'backlog_per_instance',
                            'Value': 'shlomigd_backlog_per_instance'
                        },
                    ],
                    'Unit': 'None',
                    'Value': 1.0
